...
class VerificationController extends GetxController {
  ...
  Future<void> verifyOtp() async {
    ...
    debugPrint(
      "📤 VERIFY API BODY: { email: $email, otp_code: ${pin.value.trim()} }",
    );
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // Save temporary token for profile creation only
        if (data['data'] != null && data['data']['access'] != null) {
          final tempAccessToken = data['data']['access'];
          Get.find<StorageService>().saveAccessToken(tempAccessToken);
          debugPrint(
            "⚠️ Temporary Access Token saved (will be removed after profile creation)",
          );
          debugPrint("🔑 Temp Token: $tempAccessToken");
        }
